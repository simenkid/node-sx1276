...
var createFskModem = require('./modem/fsk'),
    createLoraModem = require('./modem/lora')
    REG = require('./constants/reg');

var CONST = require('./constants/constants'),
    OPMODE = CONST.OPMODE,
    PA_PAC = CONST.PA_PAC,
    FIFO_SIZE = CONST.FIFO_SIZE,
    FXOSC_STEP = CONST.FXOSC_STEP,
    RADIO_EVENT = CONST.RADIO_EVENT,
...
Sx1276.prototype.readReg = function (regHdl, callback) {
    ...
    outPkt[0] = addr;
    outPkt[1] = 0x00;   // dummy data while reading
...
Sx1276.prototype.reset = function (callback) {
    return this.reset.high().delay(1)   // 1 ms
            .float().delay(10)          // 10 ms
            .nodeify(callback);
};
...
Sx1276.prototype.startCad = function (callback) {
    return this.modem.startCad().nodeify(callback);
};
...
Sx1276.prototype.setMaxPayloadLength = function (max, callback) {
    return this.modem.setMaxPayloadLength(max).nodeify(callback);
};
