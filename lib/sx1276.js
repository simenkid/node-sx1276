...
Sx1276.prototype.setChannel = function (freq, callback) {
    ...
    return this.writeReg(REG.FrfMsb.address, freqMSB).then(function () {
        return self.writeReg(REG.FrfMid.address, freqMID);
    }).then(function () {
        return self.writeReg(REG.FrfLsb.address, freqLSB);
    }).then(function () {
        self.channel = freq;
        return freq;
    }).nodeify(callback);
