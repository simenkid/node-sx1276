...
lora.enable = function (callback) {
    ...
    return chip.setOpMode(TODO_MODE_Sleep).then(function () {
        // turn on lora
        return chip.write('COM.OpMode', { longRangeMode: 1 });
    }).then(function () {
...
lora.readRssi = function (callback) {
    ...
    return chip.readReg('LORA.RssiValue').then(function (val) {
        var rssi = (chip.channel > TODO_RF_MID_BAND_THRESH) ? TODO_LOR_RSSI_OFFSET_HF + val : TODO_LOR_RSSI_OFFSET_LF + val;
        return rssi;
    }).nodeify(callback);
};
