...
fsk.send = function (buf, size, timeout, callback) {
    var setn = this.settings;
    setn.fskPacketHandler.NbBytes = 0;
    setn.fskPacketHandler.Size = size;

    if (this.settings.FixLen === false)
        this.writeFifo(size, 1);
    else
        this.writeReg(REG.FSK.PayloadLength, size);
    ...
    this.writeFifo(buf, this.settings.fskPacketHandler.ChunkSize);

    setn.fskPacketHandler.NbBytes += this.settings.fskPacketHandler.ChunkSize;
};
